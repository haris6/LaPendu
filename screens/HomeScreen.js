...
export default function HomeScreen({navigation}) {
    ...
    const renderItem = ({ item }) => {
        return (
            < View style={{}}>
                <TouchableOpacity
                    style={{ flexDirection: 'row', justifyContent: 'space-between', marginVertical: 7, marginRight: 5 }}
                    onPress={() => {
                        myfun(item);
                    }}
                >
                    <View style={{ flexDirection: 'row' }}>
                        ...
                        />
                        <Text style={{ fontSize: 18, fontWeight: 'bold', textAlign: 'left', marginVertical: 5, marginLeft: 5, letterSpacing: -1.2,color:'black' }}>{item.toUpperCase()}</Text>
                    </View>
                    <Image
